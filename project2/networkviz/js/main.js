...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>You must search for an institute name with a minimum of 3 letters.</i>");
        else {
...
function nodeActive(a) {
    ...
    d = "";
		for (g in e) {
			...
			f.push('<li class="membership" style="color: #eff4df"><a style="color: #eff4df" href="#' + c.name + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + c.name + "</a></li>");
		}
		return f;
	}
    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
            var d = f.attributes[attr],
                h = "";
			if (attr=="size") {
                h = '<span><strong>' + 'Co-Authorships' + ':</strong> ' + Math.round(d) + '</span><br/>'
			}
    ...
    $GP.info_data.show();
    $GP.info_p.html("Co-Authorship Affilliations:");
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
            ...
            !0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<li class="membership" style="color: #eff4df"><a style="color: #eff4df" href="#'+d.label+'" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" onclick=\"nodeActive('" + d.id + '\')" onmouseout="sigInst.refresh()">' + d.label + "</a></li>"))
        }
